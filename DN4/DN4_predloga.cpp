...
int main() {
  ...
  for (int ii=0; ii<2000; ii++)
	...
	===================================================================================
      */
      ...
      // Gauss–Seidel metoda (serijsko):
      // Klasičnega Gauss–Seidel se ne da pravilno paralelizirati po vrsticah,
      // ker je odvisen od že posodobljenih vrednosti.
      //Lahko pa bi uporabili recimo jacobi metodo za paralelno izvedbo.
      for (int jj = 0; jj < n_b; jj++)
  {
    ...
    if (A[jj][jj] == 0.0)
      {
        std::cerr << "Napaka: A[" << jj << "][" << jj << "] == 0, deljenje ni možno." << std::endl;
      }
    else
      {
        T[jj] = d / A[jj][jj];
  ...
  std::cout << "Cas od GS: " << time_duration.count() << " sekund" << std::endl;
  ...
  std::cout << "Cas izvedbe: " << time_duration.count() << " sekund" << std::endl;
